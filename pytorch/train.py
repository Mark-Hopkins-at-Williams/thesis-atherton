...
if __name__=='__main__':
    ...
    trans_dir = "\n".join(sys.argv)[9:]
    config_filepath = trans_dir + "config.json"
    ...
    BPE_DROPOUT_RATE = data["BPE_DROPOUT_RATE"]

    data = None
    with open("model_config.json", "r") as jsonfile:
        data = json.load(jsonfile)
        print("Read successful")
        jsonfile.close()
    ...
    src_tokenizer = train_tokenizer_with_algo(DATA_DIR, SRC_LANGUAGE, TGT_LANGUAGE, ALGORITHM, BPE_DROPOUT_RATE, SRC_VOCAB_SIZE)
    tokenizer = train_tokenizer_with_algo(DATA_DIR, SRC_LANGUAGE, TGT_LANGUAGE, ALGORITHM, BPE_DROPOUT_RATE, TGT_VOCAB_SIZE)
    token_transform = {}
    token_transform[SRC_LANGUAGE] = src_tokenizer
    ...
    def train_model(verbose = False):
        ...
        filepath = trans_dir + "results.txt"
        ...
        for epoch in range(1, NUM_EPOCHS+1):
            ...
            if not ((epoch + 1) % 5):  
                chrf, bleu = evaluate_bleu_chrf(transformer, dataset)
